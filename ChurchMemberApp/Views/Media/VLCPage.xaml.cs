...
namespace ChurchMemberApp.Views.Media
{
    ...
    public partial class VLCPage : ContentPage
    {
        ...
        public void downloadfile()
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(medias.filePath))
                {
                    return;
                }
                indicator.IsRunning = true;
                ...
                if (string.IsNullOrEmpty(medias.filePath)) return;
                Uri uri = new Uri(medias.filePath);
                client.OpenRead(uri);
                var bytes_total = client.ResponseHeaders["Content_Lenght"];


                client.DownloadFileAsync(uri, $"{dir}/{filename}.mp4");
        ...
        private void Client_DownloadFileCompleted(object sender, AsyncCompletedEventArgs e)
        {
            ...
            var media = App.Database.AddupdatedDownloadedMedia(new Models.DownloadedMedia(this.medias)).Result;
            if (media == 1)
            {
                message.LongAlert("Video downloaded successfully");
            }
            indicator.IsRunning = false;
            download.IsEnabled = true;
